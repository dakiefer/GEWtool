% Test that the group velocity is correctly computed by groupVel()
...
devCgDwdk = devCgDwdk(200:end,:); % remove regions of large deviation due to approximation in dwdk
assert( all(devCgDwdk <= 20, 'all') ); 
...
devCgDwdk = abs(cg - dwdk);
devCgDwdk = sort(devCgDwdk(:),'descend');
assert( max(devCgDwdk(100:end),[],'all') <= 50 )
...
devCgDwdk = abs(cg - dwdk);
devCgDwdk = sort(devCgDwdk(:),'descend');
assert( max(devCgDwdk(100:end),[],'all') <= 20 )
