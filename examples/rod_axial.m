...
% singularity at r=0 that appears in the 1/r-term. Only waves of 0th order
...
k = linspace(0, 15, 200)/r;        % wavenumbers to solve for
cyl = Cylinder(mat, [1e-3, 1e-3+r], N); tic;   % create waveguide description 
...
hT = plot(datT.k/1e3, datT.w/2/pi/1e6, 'Color', "#D95319"); % plot torsional in red
hL = plot(datL.k/1e3, datL.w/2/pi/1e6, 'Color', "#0072BD"); % plot longitudinal in blue
...
hT = plot(datT.w/2/pi/1e6, datT.w./datT.k/1e3, 'Color', "#D95319"); % plot torsional in red
hL = plot(datL.w/2/pi/1e6, datL.w./datL.k/1e3, 'Color', "#0072BD"); % plot longitudinal in blue
