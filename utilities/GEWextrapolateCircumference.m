function [ue, phi] = GEWextrapolateCircumference(u, n, Nphi)
...
% - Nphi:   (scalar, default: 60) number of points along the circumference.
% 
% Return values: 
% - ue:     (array) extrapolated field data. It has one more dimension than 'u' that
%           describes the angular dependence. The new dimension is the second one (the 
%           first one is usually the radial dependance).
% - phi:    (vector) vector of angular coordinates on which the extrapolation was
%           performed.
%
...
u = permute(u,perm);
phi = linspace(0, 2*pi, Nphi);   % angular coordinates
